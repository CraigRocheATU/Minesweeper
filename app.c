...
void load_game_menu(Game* g) {
    ...
    int count = 0;
    if (!list_save_files(paths, 128, &count) || count == 0) { printf("No save files found.\n"); return; }
    printf("\nSaved Games:\n");
...
int app_main(void) {
    // ensure sample login file exists
    FILE* lf = fopen("login.txt", "r");
    if (!lf) {
        lf = fopen("login.txt", "w");
        if (lf) {
            fprintf(lf, "alice,alice123,Alice\nbob,bob123,Bob\ncarol,carol123,Carol\n");
            fclose(lf);
        }
    }
    else fclose(lf);
    ...
    for (;;) {
        ...
        if (choice == 1) {
            ...
            if (!db) {
                ...
                if (!login_players(db, &np, user, disp)) { printf("Login canceled.\n"); continue; }
            }
